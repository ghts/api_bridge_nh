...
func f콜백_접속(c *C.LOGINBLOCK) {
	...
	// 콜백 대기 응답
	for _, 대기_항목 := range 대기항목_저장소.G대기항목_모음() {
		if 대기_항목.내용.TR구분() == lib.TR접속 {
			대기_항목.내용.G질의().S응답(lib.New채널_메시지(lib.TR응답_완료, 로그인_정보))
			대기항목_저장소.S삭제(대기_항목.키)
			lib.F체크포인트("f콜백_접속() 대기 항목 삭제")
		}
	}

	lib.F체크포인트("f콜백_접속() 완료")
}
...
func f콜백_접속_해제() {
	...
	for _, 대기_항목 := range 대기항목_저장소.G대기항목_모음() {
		switch 대기_항목.내용.TR구분() {
		case lib.TR접속_해제: // 접속 해제 요청이 성공했으므로, 에러가 아님
			대기_항목.내용.G질의().S응답(lib.New채널_메시지(lib.TR응답_완료))
		case lib.TR종료, lib.TR접속: // 종료 작업의 일부이므로 에러가 아님.
			continue
		default: // 나머지 모든 경우에 대해서 에러 회신.
			lib.F문자열_출력("접속 해제 콜백. 예상하지 못한 경우. '%v'", 대기_항목.내용.TR구분())
			대기_항목.내용.G질의().S응답(lib.New채널_메시지_에러(접속_해제_에러))
		}
	}
...
func f콜백_메시지(c *C.OUTDATABLOCK) {
	...
	// 해당되는 조회 질의가 존재하면 처리.
	대기_항목 := 대기항목_저장소.G대기항목(데이터.M식별번호)

	if 대기_항목 != nil {
		대기_항목.내용.G질의().S응답(lib.New채널_메시지(lib.TR응답_메시지, 데이터.M메시지_코드, 데이터.M메시지_내용))
		return
	}
...
func f콜백_TR데이터(c *C.OUTDATABLOCK) {
	...
	변환값, 에러 := lib.New바이트_변환_매개체(대기_항목.내용.G변환_형식(), 응답값)
	if 에러 != nil {
		lib.F에러_출력(에러)
	}

	대기_항목.내용.G질의().S응답(lib.New채널_메시지(lib.TR응답_데이터, 변환값))
}
...
func f콜백_TR완료(c *C.OUTDATABLOCK) {
	...
	대기_항목.내용.G질의().S응답(lib.New채널_메시지(lib.TR응답_완료, 변환값))
...
func f에러_콜백_처리(c *C.OUTDATABLOCK) {
	...
	대기_항목.내용.G질의().S응답(lib.New채널_메시지_에러(수신_에러))
}
