...
// 주식 현재가 조회 (c1101)
//----------------------------------------------------------------------//
func NewTc1101InBlock(종목코드 string) *C.char {
	g := new(Tc1101InBlock)
...
// ETF 현재가 조회 (c1151)
//----------------------------------------------------------------------//
func NewTc1151InBlock(종목코드 string) *C.char {
	g := new(Tc1151InBlock)
	lib.F바이트_복사_문자열(g.Lang[:], "k")
	lib.F바이트_복사_문자열(g.Code[:], 종목코드)
...
// 주식 매도(c8101)
//----------------------------------------------------------------------//
func NewTc8101InBlock(주문 *lib.S질의값_정상주문) *C.char {
	switch {
...
// 주식 매수(c8102)
//----------------------------------------------------------------------//
func NewTc8102InBlock(주문 *lib.S질의값_정상주문) *C.char {
	switch {
	case 주문.M증권사 != lib.P증권사_NH:
		lib.F패닉("잘못된 증권사. '%v'", 주문.M증권사)
...
// 주식 정정 주문 (c8103)
// ----------------------------------------------------------------------//
func NewTc8103InBlock(주문 *lib.S질의값_정정주문) *C.char {
...
// 주식 취소 주문 (c8104)
// ----------------------------------------------------------------------//
func NewTc8104InBlock(주문 *lib.S질의값_취소주문) *C.char {
	if 주문.M증권사 != lib.P증권사_NH {
		lib.F패닉("잘못된 증권사. %v", 주문.M증권사)
	}
