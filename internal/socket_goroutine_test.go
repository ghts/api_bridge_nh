...
func u1_k1_수신_테스트(t *testing.T, 질의_인수 *sRT질의_인수) {
	...
	for _, 데이터 := range 데이터_모음 {
		...
		lib.F테스트_참임(t, s.M거래비중 >= 0, s.M거래비중)
		lib.F테스트_참임(t, s.M거래비중 <= 100, s.M거래비중)
...
func TestTR소켓_접속(t *testing.T) {
	...
	접속_시각 := 로그인_정보.M접속_시각
	p지금 := time.Now()
	p3분전 := p지금.Add(-1 * lib.P3분)
	p3분후 := p지금.Add(lib.P3분)

	lib.F테스트_참임(t, 접속_시각.After(p3분전), p지금, 접속_시각)
	lib.F테스트_참임(t, 접속_시각.Before(p3분후), p지금, 접속_시각)
	lib.F테스트_참임(t, f접속됨())
}
