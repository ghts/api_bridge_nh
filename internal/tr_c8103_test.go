...
func TestC8103_주식_정정주문(t *testing.T) {
	...
	원주문번호 := 주문번호_정상주문
	가격_정정주문 := 일일_종목정보.M하한가
	수량_정정주문 := int64(0)

	// 정정
	반복_횟수 := lib.F임의값_생성기().Intn(int(수량_정상주문) - 1)
	for i := 0; i < 반복_횟수; i++ {
		lib.F문자열_출력("정정주문 : %v", i)
		switch 가격_정정주문 {
		case 일일_종목정보.M하한가:
			가격_정정주문 = 일일_종목정보.M하한가 + int64(최소_호가단위)
		default:
			가격_정정주문 = 일일_종목정보.M하한가
		}

		수량_정정주문 = 수량_정상주문 - int64(i)
		원주문번호 = f정정주문_테스트_도우미(t, 계좌번호, 종목, lib.P잔량, 원주문번호, 가격_정정주문, 수량_정정주문)
	}

	// 전량 취소
	lib.F문자열_출력("\n전량 취소")
	f취소주문_테스트_도우미(t, 계좌번호, 종목, lib.P잔량, 원주문번호, 0)
