...
func f접속_TR처리(질의 lib.I채널_질의) {
	...
	if f접속_진행_중() {
		lib.F체크포인트("접속 시도 진행 중임.")
		질의.S응답(lib.New채널_메시지_에러("접속 시도 진행 중임."))
		return
	}

	lib.F체크포인트("접속 전 확인작업 완료")
	...
	lib.F체크포인트("f접속_TR처리() 완료. f콜백_접속()에서 계속 진행됨.")

	// 이후 f콜백_접속()에서 계속 진행됨.
}

func f접속해제_TR처리(질의 lib.I채널_질의) {
	lib.F체크포인트("f접속해제_TR처리() 시작")
...
func f정기_점검() {
	...
	if !lib.F테스트_모드_실행_중() {
		f접속_확인()    // 주기적인 접속 확인이 접속 테스트에 방해가 됨.
	}
}
...
func f접속_확인() error {
	...
	for i := 0; i < 10; i++ {
		...
		lib.F문자열_출력("%s : %v", 응답.G에러(), i)
		lib.F변수값_확인(응답.G값_모음())
	}

	if !f접속됨() {
		lib.F문자열_출력("접속 확인 : 접속 최종 실패")
	}
